...
namespace ProjectKongor.Tests.Libs;
public class PHPTest
{
	...
	public void SerializeBasicObject()
	{
		...
		Assert.Equal(
			"a:2:{s:4:\"auth\";s:17:\"Account Not Found\";i:0;b:0;}",
			PHP.Serialize(authFailedResponse)
		);
	}
	...
	{
		Assert.Equal("a:0:{}", PHP.Serialize(new Nullable()));
	}
	...
	public void ComplexDictionaryTypes()
	{
		...
		Assert.Equal("a:1:{i:123;i:321;}", PHP.Serialize(dictionary));
	}

	[Fact]
	public void ListOfStrings()
	{
		List<string> list = new() { "hello" };
		Assert.Equal("a:1:{i:0;s:5:\"hello\";}", PHP.Serialize(list));
	}

	[Fact]
	public void ListOfInts()
	{
		List<int> list = new() { 123 };
		Assert.Equal("a:1:{i:0;i:123;}", PHP.Serialize(list));
	}

	[Fact]
	public void SerializeEnum()
	{
		List<AccountType> listOfEnums = new() { AccountType.Staff };
		List<int> listOfInts = new() { (int)AccountType.Staff };
		Assert.Equal(PHP.Serialize(listOfInts), PHP.Serialize(listOfEnums));
	}
	...
	public void SerializeUTF8()
	{
		...
		Assert.Equal("s:14:\"coup de grâce\";", PHP.Serialize(utf8));
	}
	...
	public void StaticPropertiesAreNotSerialized()
	{
		...
		Assert.Equal("a:0:{}", PHP.Serialize(data));
	}
	...
	{
		Assert.Equal(
			"a:6:{s:1:\"b\";b:1;s:1:\"i\";i:2;s:1:\"l\";i:3;s:1:\"f\";d:4.5;s:1:\"d\";d:6.7;s:1:\"s\";s:3:\"str\";}",
			PHP.Serialize(new PrimitiveValues())
		);
	}
	...
	public void SerializeStringArray()
	{
		...
		Assert.Equal("a:3:{i:0;s:1:\"1\";i:1;s:1:\"2\";i:2;s:1:\"3\";}", PHP.Serialize(array));
	}
	...
	public void SerializeObjectArray()
	{
		...
		Assert.Equal("a:1:{i:0;N;}", PHP.Serialize(array));
	}

	[Fact]
	public void RareCollectionType()
	{
		Dictionary<long, long> d = new() { [42] = 24 };
		Assert.Equal("a:1:{i:0;i:42;}", PHP.Serialize(d.Keys));
		Assert.Equal("a:1:{i:0;i:24;}", PHP.Serialize(d.Values));
	}

	[Fact]
	public void SerializeHashSetOfInts()
	{
		HashSet<int> h = new() { 42 };
		Assert.Equal("a:1:{i:0;i:42;}", PHP.Serialize(h));
	}

	[Fact]
	public void SerializeHashSetOfStrings()
	{
		HashSet<string> h = new() { "test" };
		Assert.Equal("a:1:{i:0;s:4:\"test\";}", PHP.Serialize(h));
	}
}
